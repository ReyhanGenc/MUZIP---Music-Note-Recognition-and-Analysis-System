...
class AudioAnalyzer:
    def __init__(self, data=None, rate=44100, fft_processor=None, note_detector=None,
    ...
    def get_db(self, rms_val):
        if rms_val <= 0: return -100
        return 20 * np.log10(rms_val)

    def process_chunk(self, chunk, idx, db_threshold=-40):
        e = self.rms(chunk)
        db = self.get_db(e)
